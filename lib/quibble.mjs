...
export async function resolve (specifier, context, nextResolve) {
  ...
  // All of the branches below are driven entirely by markers on `specifier`
  // itself, so they run unconditionally - regardless of `__quibbleSuppressed`
  // - and are never at risk of being skipped by a stale suppression flag.
  ...
  // middle of our own `nextResolve` call above (e.g. while walking
  ...
  //
  // `context.__quibbleSuppressed` (set/cleared around `nextResolve` in
  // `resolve()` above) flags that window. Node can also reuse/pool the same
