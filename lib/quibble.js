...
quibble.esm = async function (importPath, namedExportStubs, defaultExportStub) {
  ...
  const modulePath = importPathIsBareSpecifier
    ...
    : isAbsolutePath
      ? importPath
      : (pathPrefix + path.resolve(path.dirname(callerFile), importPath))
  const fullModulePath = process.platform !== 'win32' ? modulePath : modulePath.split(path.sep).join('/')
...
quibble.esmImportWithPath = async function esmImportWithPath (importPath) {
  ...
  const fullImportPath = importPathIsBareSpecifier
    ? importPath
    : (process.platform !== 'win32' ? modulePath : modulePath.split(path.sep).join('/'))
